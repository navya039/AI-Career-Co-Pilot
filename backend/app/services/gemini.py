...
async def generate_ai_suggestions(resume_text: str, job_description_text: str, missing_skills: list) -> str:
    if not model:
...
async def generate_bullet_point_suggestion(bullet_point: str, improvement_type: str = "make it more impactful") -> str:
    if not model:
        return "Error: Gemini API is not configured."
...
async def generate_learning_roadmap(skill: str) -> str:
    ...
    **Please format your entire response using Markdown.** Use headings for sections, subheadings, bold text, and bullet points to make the plan clear and easy to read.
    """
    try:
        response = await model.generate_content_async(prompt)
        return response.text.strip()
...
async def generate_interview_questions(resume_text: str, job_description_text: str) -> str:
    if not model:
        return "Error: Gemini API is not configured."
